...
export const Route = createFileRoute("/studies")({
  head: () => ({
    meta: [{ title: "Estudios — Victor De Ascencao - QA" }],
  }),
  component: Studies,
});
...
function PDFCard({
  ...
  pdf: { title: string; file: string; thumb?: string };
  onOpen: (p: any) => void;
}) {
  return (
            ...
            {loading ? (
              ...
              <div className="rounded-2xl border border-border bg-card p-8">
                <p className="text-muted-foreground">
                  Aún no hay documentos. Agrega PDFs en public/studies y opcionalmente crea
                  public/studies/index.json con metadatos.
                </p>
              </div>
            ) : (
              ...
              <div className="relative z-10 w-full max-w-4xl rounded-2xl overflow-hidden bg-background">
                <div className="flex items-center justify-between border-b border-border p-4">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button onClick={() => setOpenPdf(null)} className="rounded-md p-2">
                      Cerrar
                    </button>
                  </div>
                </div>
